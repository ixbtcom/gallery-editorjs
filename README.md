...
- **Загрузка файлов** — с устройства, по URL или из буфера обмена
...
| `generation.source` | `{ name: string, url: string }` | Источник и ссылка, предзаполняемые для AI-изображений |

При включённой AI-генерации выбранный вариант перед сохранением показывает те же поля, что и обычное изображение Gallery: подпись, источник и ссылку на источник. Редактор может изменить их до вставки изображения.

Изображение из буфера можно добавить отдельной кнопкой или вставить прямо в поле URL — в обоих случаях оно загружается в текущую галерею. Для каждого изображения доступны настройки `Не ограничивать по высоте`, `Не оптимизировать` и `Не применять оформление`. При отключённой оптимизации Gallery сохраняет исходный URL, не применяет crop и не генерирует responsive-варианты.
